...
//c----------------------------------------------------
    ...
    MPI_Bcast(htmp, 3, MPI_INT, 0, MPI_COMM_WORLD);
...
void Schemes_Choose_ID(SCHEME_CHOOSE *scheme){
    ...
    pthread_create(&thread_handles[8], NULL, invis_choose_SCHEME_HYBRIDAUTO, (void*)(*scheme).invis);


    pthread_create(&thread_handles[9], NULL, vis_choose_CD6, (void*)(*scheme).vis);
    pthread_create(&thread_handles[10], NULL, vis_choose_CD8, (void*)(*scheme).vis);

    for(long thread = 0; thread < 11; thread++)
