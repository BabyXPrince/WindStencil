...
void OCFD_Dx0_bound(cudaField f , cudaField fx , cudaJobPackage job_in , dim3 blockdim_in, hipStream_t *stream){ 
	CHECK_X(
		{
...
void OCFD_Dy0_bound(cudaField f , cudaField fx , cudaJobPackage job_in , dim3 blockdim_in, hipStream_t *stream){
	CHECK_Y(
		{
...
void OCFD_Dz0_bound(cudaField f , cudaField fx , cudaJobPackage job_in , dim3 blockdim_in, hipStream_t *stream){
	CHECK_Z(
		{
...
void OCFD_bound(dim3 *flagxyzb, int boundp, int boundm, cudaJobPackage job){
	...
	switch(flagxyzb->x){
		case 1:
		if(npx == 0 && job.start.x == LAP && boundp == 1) flagxyzb->x = 4;
		if(npx == NPX0-1 && job.end.x == nx_lap && boundm == 1) flagxyzb->y = 4;
		break;

		case 2:
		if(npy == 0 && job.start.y == LAP && boundp == 1) flagxyzb->x = 5;
		if(npy == NPY0-1 && job.end.y == ny_lap && boundm == 1) flagxyzb->y = 5;
		break;

		case 3:
		if(npz == 0 && job.start.z == LAP && boundp == 1) flagxyzb->x = 6;
