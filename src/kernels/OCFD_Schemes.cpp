...
__device__ int get_data0_kernel(int flagxyz, dim3 *coords, cudaField pf, REAL *stencil, int ka1, int kb1, cudaJobPackage job){
	...
	switch(flagxyz){
		...
		case 5:
		{
			unsigned int x = coords->x = blockDim.y * blockIdx.y + threadIdx.y;
			unsigned int y = coords->y = blockDim.x * blockIdx.x + threadIdx.x;
			unsigned int z = coords->z = blockDim.z * blockIdx.z + threadIdx.z;
		...
		case 6:
		{
			...
			unsigned int z = coords->z = blockDim.x * blockIdx.x + threadIdx.x;
...
__device__ int get_data_kernel(int flagxyz, dim3 *coords, cudaSoA f, int num, REAL *stencil, int ka1, int kb1, cudaJobPackage job){
	...
	switch(flagxyz){
		...
		{
			unsigned int x = coords->x = blockDim.y * blockIdx.y + threadIdx.y;
			unsigned int y = coords->y = (blockDim.x-1) * blockIdx.x + threadIdx.x;
			unsigned int z = coords->z = blockDim.z * blockIdx.z + threadIdx.z;

			if(x < (job.end.x-job.start.x) && y < (job.end.y-job.start.y) && z < (job.end.z-job.start.z)){
				for(int i = ka1; i <= kb1; i++){
					stencil[i-ka1] = get_SoA_LAP(f, x, y+i, z, num, offset);
				}
				return 2;
			}
		}
		...
		case 6:
		{
			unsigned int x = coords->x = blockDim.z * blockIdx.z + threadIdx.z;
			unsigned int y = coords->y = blockDim.y * blockIdx.y + threadIdx.y;
			unsigned int z = coords->z = (blockDim.x-1) * blockIdx.x + threadIdx.x;

			if(x < (job.end.x-job.start.x) && y < (job.end.y-job.start.y) && z < (job.end.z-job.start.z)){
				for(int i = ka1; i <= kb1; i++){
					stencil[i-ka1] = get_SoA_LAP(f, x, y, z+i, num, offset);
				}
				return 3;
			}
		}
...
__device__ void put_du_p_kernel(dim3 flagxyz, dim3 coords, REAL tmp_r, REAL tmp_l, cudaSoA du, int num, cudaField Ajac, cudaJobPackage job){
	...
	switch(flagxyz.x){
		case 1:
		case 4:
		if(flagxyz.x == 4 && flagxyz.z == 1 && coords.x == 1){
		...
		case 2:
		case 5:
		if(flagxyz.x == 5 && flagxyz.z == 1 && coords.y == 1){
			get_SoA(du, x-LAP, y-LAP, z-LAP, num) += 0;
		}else{
		...
		case 3:
		case 6:
		if(flagxyz.x == 6 && flagxyz.z == 1 && coords.z == 1){
			get_SoA(du, x-LAP, y-LAP, z-LAP, num) += 0;
		}else{
...
__device__ REAL OCFD_weno7_SYMBO_kernel_P(int WENO_LMT_FLAG, REAL *stencil){
	...
	if(WENO_LMT_FLAG == 1){
		S0 = abs(stencil[1] - stencil[0]) + abs(stencil[2] - stencil[1]) + abs(stencil[3] - stencil[2]);
		S1 = abs(stencil[2] - stencil[1]) + abs(stencil[3] - stencil[2]) + abs(stencil[4] - stencil[3]);
		S2 = abs(stencil[3] - stencil[2]) + abs(stencil[4] - stencil[3]) + abs(stencil[5] - stencil[4]);
		S3 = abs(stencil[4] - stencil[3]) + abs(stencil[5] - stencil[4]) + abs(stencil[6] - stencil[5]);
		S4 = abs(stencil[5] - stencil[4]) + abs(stencil[6] - stencil[5]) + abs(stencil[7] - stencil[6]);
...
__global__ void OCFD_weno7_SYMBO_P_kernel(int WENO_LMT_FLAG, dim3 flagxyzb, cudaSoA f, cudaSoA du, cudaField Ajac, cudaJobPackage job){
	...
	for(int i = 0; i < 5; i++){
		...
		int flag = get_data_kernel(flagxyzb.x, &coords, f, i, &stencil[0], ia1, ib1, job);
...
__device__ REAL OCFD_weno7_SYMBO_kernel_M(int WENO_LMT_FLAG, REAL *stencil){
	...
	if(WENO_LMT_FLAG == 1){
		S0 = abs(stencil[7] - stencil[6]) + abs(stencil[6] - stencil[5]) + abs(stencil[5] - stencil[4]);
		S1 = abs(stencil[6] - stencil[5]) + abs(stencil[5] - stencil[4]) + abs(stencil[4] - stencil[3]);
		S2 = abs(stencil[5] - stencil[4]) + abs(stencil[4] - stencil[3]) + abs(stencil[3] - stencil[2]);
		S3 = abs(stencil[4] - stencil[3]) + abs(stencil[3] - stencil[2]) + abs(stencil[2] - stencil[1]);
		S4 = abs(stencil[3] - stencil[2]) + abs(stencil[2] - stencil[1]) + abs(stencil[1] - stencil[0]);
...
__device__ REAL minmod2(REAL x1, REAL x2){
	...
	REAL minmod2 = 0.5*(sign(1.0, x1) + sign(1.0, x2))*fmin(abs(x1),abs(x2));
...
__device__ REAL minmod4(REAL x1, REAL x2, REAL x3, REAL x4){
	...
	minmod4 = minmod4*abs(0.5*(sign(1.0, x1) + sign(1.0, x3)));
	minmod4 = minmod4*abs(0.5*(sign(1.0, x1) + sign(1.0, x4)));
...
__global__ void OCFD_NND2_P_kernel(dim3 flagxyzb, cudaSoA f, cudaSoA du, cudaField Ajac, cudaJobPackage job){
	...
	for(int i = 0; i < 5; i++){
		...
		if(flag != 0){
			...
			if(flag != 0) tmp_r = OCFD_NND2_kernel_P(&stencil[0]);
...
__global__ void OCFD_NND2_M_kernel(dim3 flagxyzb, cudaSoA f, cudaSoA du, cudaField Ajac, cudaJobPackage job){
	...
	for(int i = 0; i < 5; i++){
		...
		if(flag != 0){
			...
			if(flag != 0) tmp_r = OCFD_NND2_kernel_M(&stencil[0]);
...
__device__ REAL OCFD_weno5_kernel_P(REAL *stencil){
	...
	REAL a0 = ((12.0*ep + S1)*(12.0*ep + S1))*((12.0*ep + S2)*(12.0*ep + S2));
	...
	REAL a1 = ((12.0*ep + S0)*(12.0*ep + S0))*((12.0*ep + S2)*(12.0*ep + S2));
...
__device__ REAL OCFD_weno5_kernel_M(REAL *stencil){
	...
	REAL a1 = 1.0/((12.0*ep + S1)*(12.0*ep + S1));
