...
void comput_Jac3d()
{
    ...
    cudaField *xi; new_cudaField(&xi , nx,ny,nz);
    cudaField *xj; new_cudaField(&xj , nx,ny,nz);
    cudaField *xk; new_cudaField(&xk , nx,ny,nz);
    cudaField *yi; new_cudaField(&yi , nx,ny,nz);
    cudaField *yj; new_cudaField(&yj , nx,ny,nz);
    cudaField *yk; new_cudaField(&yk , nx,ny,nz);
    cudaField *zi; new_cudaField(&zi , nx,ny,nz);
    cudaField *zj; new_cudaField(&zj , nx,ny,nz);
    cudaField *zk; new_cudaField(&zk , nx,ny,nz);
...
__global__ void boundary_Jac3d_kernal_y_ramp_wall_kernel(
    ...
    if( x < job.end.x && y < job.end.y && z < job.end.z){
        if( get_Field_LAP(xx, x, LAP, z) <= 0.0){
            ...
            REAL tmpxx = abs(-cos(2*seta) + sin(2*seta)*dy/dx);
            REAL tmpyy = abs(cos(2*seta) + dx/dy*sin(2*seta));
...
__global__ void boundary_Jac3d_kernal_z_cone_wall_kernel(
    ...
    if( x < job.end.x && y < job.end.y && z < job.end.z){
        if( get_Field_LAP(xx, x, y, LAP) <= 0.0){
            ...
            REAL tmpxx = abs(-cos(2*seta1) + sin(2*seta1)*dz/dx);
            REAL tmpzz = abs(cos(2*seta1) + dx/dz*sin(2*seta1));
            ...
            REAL tmpxx = abs(-cos(2*(seta1+seta2)) + sin(2*(seta1+seta2))*dz/dx);
            REAL tmpzz = abs(cos(2*(seta1+seta2)) + dx/dz*sin(2*(seta1+seta2)));
